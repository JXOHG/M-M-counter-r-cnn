...
class ParameterOptimizer:
    ...
    def preprocess_image(self, tile_grid_size, blur_sigma):
        # Apply CLAHE
        clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=tile_grid_size)
        clahe_image = clahe.apply(self.gray)
        
        # Apply Gaussian Blur
        blurred = cv2.GaussianBlur(clahe_image, (0, 0), blur_sigma)
        
        return blurred

    def detect_circles(self, processed_image):
    ...
    def optimize_parameters(self):
        ...
        for params in ParameterGrid(param_grid):
            processed_image = self.preprocess_image(params['tile_grid_size'], params['blur_sigma'])
            ...
            print(f"Parameters: {params}, Confidence: {confidence}")
...
class ImageProcessor:
    ...
    def preprocess_image(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        try:
            ...
            # Use parameter tuner if available, otherwise use default kernel size
            if self.parameter_tuner is not None:
                try:
                    kernel_size = self.parameter_tuner.predict_kernel_size(image)
                    # Ensure kernel size is odd
                    kernel_size = max(3, kernel_size + (1 - kernel_size % 2))
                    blur_kernel = (kernel_size, kernel_size)
                except Exception as e:
                    self.logger.warning(f"Parameter tuning failed, using default: {str(e)}")
            ...
            blurred = cv2.GaussianBlur(image, blur_kernel, 0.25)
    ...
    def preprocess_image(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        try:
            ...
            # Apply Gaussian blur to reduce noise
            blurred = cv2.GaussianBlur(image, self.blur_kernel_size, 0.25) # was 0
            ...
            # blur2 = cv2.GaussianBlur(hist, (31,31), cv2.BORDER_DEFAULT)
...
class MMDetector:
    ...
    def process_image(self, gray: np.ndarray, hsv: np.ndarray) -> Tuple[np.ndarray, List[str]]:
        ...
        try:
            # Detect circles
            circles = self.detect_circles(gray) # Change to Blurred?
